...
#else
#define dispatch_atomic_xchg(p, n)	__sync_lock_test_and_set((p), (n)) // 将*p设为value并返回*p操作之前的值。
#endif
#define dispatch_atomic_cmpxchg(p, o, n)	__sync_bool_compare_and_swap((p), (o), (n)) // 这两个函数提供原子的比较和交换：如果*p == o,就将n写入*p(p代表地址，o代表oldValue，n代表newValue)
#define dispatch_atomic_inc(p)	__sync_add_and_fetch((p), 1) // 先自加1，再返回
#define dispatch_atomic_dec(p)	__sync_sub_and_fetch((p), 1) // 先自减1，再返回
#define dispatch_atomic_add(p, v)	__sync_add_and_fetch((p), (v)) // 先自加v，再返回
#define dispatch_atomic_sub(p, v)	__sync_sub_and_fetch((p), (v)) // 先自减v，再返回
#define dispatch_atomic_or(p, v)	__sync_fetch_and_or((p), (v)) // 先返回，再进行或运算
#define dispatch_atomic_and(p, v)	__sync_fetch_and_and((p), (v)) // 先返回，再进行与运算
